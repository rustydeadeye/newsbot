...
def test_fallback_text_uses_clean_source_name() -> None:
    ...
    assert text == "RBI keeps repo rate unchanged"
...
def test_macro_template_prefers_rbi_releases_language() -> None:
    ...
    assert text == "RBI releases the second 3-day Variable Rate Repo (VRR) auction held on March 30, 2026"
...
def test_bonus_template_uses_ratio_and_date() -> None:
    ...
    assert text == "IRB: bonus issue ratio 1:1; ex-date March 30, 2026"
...
def test_fund_notice_template_stays_factual() -> None:
    ...
    assert text == "UTI Banking and PSU Fund Direct Plan Halfyearly IDCW reinvestment (9002325)"
